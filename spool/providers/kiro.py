...
Kiro is a VS Code fork, so it follows the standard `state.vscdb` SQLite
layout under `~/Library/Application Support/Kiro/User/`. We try the same
chat/composer/agent data shapes used by Cursor and Windsurf — they're all
derived from the same upstream chat-panel schema. If Kiro introduces its
own keys later, add them to `_CHAT_KEYS` / `_AGENT_KEY_PREFIXES` below.
"""
...
# Candidate base dirs — macOS primary, linux/windows-style fallbacks, and
# any dotfile-style home dir we've seen for AWS tooling.
_CANDIDATE_BASES = [
...
_AGENT_KEY_PREFIXES = ("composerData:", "agentData:", "kiroAgent:", "flowData:")
    ...
    def discover_session_files(self, data_path: Path | None = None) -> list[Path]:
        ...
        for base in bases:
            ...
            for vscdb in ws.rglob("state.vscdb"):
                if _has_kiro_data(vscdb):
                    files.append(vscdb)
            gdb = base / "globalStorage" / "state.vscdb"
            if gdb.exists() and _has_kiro_data(gdb):
                files.append(gdb)
...
def _has_kiro_data(db_path: Path) -> bool:
    try:
        conn = sqlite3.connect(f"file:{db_path}?mode=ro", uri=True)
        cur = conn.cursor()
        for pattern in ("%aichat%", "%aiChat%", "%kiro%", "%chat.data%"):
    ...
    if not title:
        ...
        if first:
            ...
            if len(first.content) > 80:
                title += "..."
    return ParsedSession(
        session_id=f"kiro-agent-{agent_id}",
        project=project,
        messages=messages,
